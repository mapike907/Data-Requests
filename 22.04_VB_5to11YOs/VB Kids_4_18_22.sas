...
/**		Q: How do cases and hospitalizations among 5-11 YOs compare to those 12+?					**/
/**																									**/															   
...
Data Adults_COVID;
	...
	if age_at_reported >= 12 then output;

RUN;

PROC FREQ data=adults_covid;
	tables age_at_reported;
	title 'age at reported';
RUN; 
...
Data archive.All_cases_4_18_22;
	set kids_COVID
		adults_COVID;
...
DATA Pop_Vax_UnVax;
	Set WORK.FILTER_FOR_VAXUNVAX_ADDTNL_AGE;
 	...
 	if age >= 5 and age <= 11 then agegrp = '5-11';
		else if age >= 12 then agegrp = '12+';
		else if age < 5 then delete; 
...
DATA population_210;
	...
	keep date agegrp Sum_total_fulvax_add Sum_total_fully_vax Sum_total_unvaxed; 
